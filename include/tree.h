 ...
 * 分配內存並初始化樹根為 NULL。
 *
 * @return 指向新二元樹的指針，如果分配失敗則返回 NULL
 */
Tree* create_tree();
 ...
 * @brief 插入節點到二元樹
 *
 * 根據數據值插入節點，保持二元搜索樹的性質。
 *
 * @param tree 指向要操作的二元樹
 * @param data 要插入的節點數據
 ...
 * @brief 釋放二元樹佔用的內存
 *
 * 釋放二元樹中所有節點的內存，然後釋放樹本身。
 *
 * @param tree 指向要釋放的二元樹
 */
